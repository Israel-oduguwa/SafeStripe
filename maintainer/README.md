...
Run `npm pack --dry-run` and inspect the file list. Run `npm pack` to produce the installable archive. Before publishing, test that archive in a separate app with `npm install /absolute/path/to/israeloduguwa-safestripe-0.2.0.tgz`. After publishing, repeat the consumer check using the exact version from the registry.

The repository URL and npm scope are separate. Verify publishing access to the `@israeloduguwa` npm scope, enable npm account protection, and configure trusted publishing or a narrowly scoped automation token before publishing. `npm publish --access public` publishes a public artifact; do not run it until you have reviewed the package, license, release notes and passing CI. No npm release is performed by these instructions.
